function plotPARAFAC4(X, model, varExps, choice, individual_mode_metadata, feature_mode_metadata, visitXlabel, usedVisits, featureOffset, title, path)
...
varExp = varExps(choice);
meanVarExp = mean(varExps);
stdVarExp = std(varExps);
sgtitle(title + ", variance explained =  " + varExp + "% (" + meanVarExp + " +/- " + stdVarExp + "%)");
saveas(gcf, path)
close()
end