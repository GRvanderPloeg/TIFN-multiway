function plotBootstrappedPARAFAC(As, Bs, Cs, varExps, individual_mode_metadata, feature_mode_metadata, visitXlabel, featureOffset, title, path)
...
for i=0:(numFactors-1)
    ...
    feature_mode = [mean(Bdata,2) std(Bdata,0,2) feature_mode_metadata];
    feature_mode = sortrows(feature_mode, 3:size(feature_mode,2), "MissingPlacement","last");
    feature_mode = [feature_mode(:,1:2) feature_mode(:,2+featureOffset)];
